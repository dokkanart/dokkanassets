...
--manba1 = entryEffectLife(  0, SP_01, 2,  0x40,  0,  1,  0,  -54); -- オーラ
entryFade( 0, 1, 5, 1, fcolor_r, fcolor_g, fcolor_b, 100);     -- white fade
entryFadeBg( 5, 0, BUFFER+300, 0, 10, 10, 10, 180);       -- ベース暗め　背景

setVisibleUI(5, 0);
manba1 = entryEffectLife(  5, SP_01,25, 0,  -1,  0,  0,  -54); -- オーラ
manba2 = entryEffect(  30, SP_02, 0,  -1,  0,  0,  -54); -- オーラ
playSe( 40, SE_06);
--setEffAlphaKey(54, manba1, 255);
--setEffAlphaKey(54, manba2, 255);

--setEffScaleKey( 1, manba1, 1.5, 1.5);
--setEffAlphaKey(0, manba1, 255);

--setEffScaleKey( 2, manba2, 1.5, 1.5);
--setEffAlphaKey(1, manba2, 255);
--setEffAlphaKey(2, manba1, 0);
--[[
...
manba3 = entryEffectLife(  80, SP_08, 90, 0,  -1,  0,  0,  -54); -- オーラ
--setEffAlphaKey(54, manba1, 0);
--setEffAlphaKey(54, manba2, 0);
...
--setEffAlphaKey(65, manba3, 0);
playSe( 90, SE_05);
shuchusen = entryEffectLife( 90, 906, 80, 0,  -1, 0,  0,  0);   -- 集中線
...
--changeAnime( 170, 0, 31);                                    -- かめはめ発射ポーズ

shunkan = entryEffectLife( 170, 700, 20, 0, -1, 0, 0, 0); 
--setDisp( 145, 0, 0);
...
playSe( BUFFER+260, SE_07);

spname = entryEffect( BUFFER+260, 1508,  0,  -1,  0,  0,  0);               -- かめはめ波文字
...
changeAnime( BUFFER+260, 1, 104);                       -- 立ち
changeAnime( BUFFER+260, 0, 31);                       -- 立ち

setAnimeLoop(BUFFER+261, 1, 1);
setMoveKey(  BUFFER+259,  0,  0,  0,   0);
setMoveKey(  BUFFER+260,  0,  -100,  0,   0);

setMoveKey(  BUFFER+260, 1, 700,  0,  0);
setDisp( BUFFER+268, 0, 0);

setDisp( BUFFER+268, 1, 1);
setMoveKey(  BUFFER+278,  1,  100,  0,   0);

shunkan2 = entryEffectLife( BUFFER+280, 700, 30, 0x80, -1, 0, -100, 0); 
setEffAlphaKey(	BUFFER+280,shunkan2, 255);

setDisp( BUFFER+280, 0, 1);
--playSe( 300, SE_06);

setMoveKey(  BUFFER+285, 0, -100,  0,  0);



entryFade( BUFFER+285, 7, 10, 5, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
setMoveKey(  BUFFER+285, 0, -50,  0,  0);


setEffAlphaKey(	BUFFER+288,shunkan2, 0);

setEffAlphaKey(	BUFFER+288,spname, 1);
------------------------------------------------------
-- 攻撃(100F)
------------------------------------------------------
setDisp( BUFFER+289+5, 0, 0);
setDisp( BUFFER+289, 1, 1);
changeAnime( BUFFER+290, 1, 104);                     -- ガード
...
-- ** エフェクト等 ** --
setShakeChara( SP_ATK01+35, 1, 99, 20);
entryFadeBg( SP_ATK01+35, 0, 36, 0, 10, 10, 10, 150);          -- ベース暗め　背景
entryEffectLife( SP_ATK01+35, 921, 36, 0x80,  -1,  0,  0,  0); -- 流線斜め
--[[
...
playSe( SP_ATK01+35+diff01, SE_09);
...
eff02 = entryEffect( SP_ATK01+43+diff01, SP_04,  0, -1,  0,  0,  0);   -- 攻撃3
...
-- 書き文字エントリー
ct = entryEffectLife( SP_ATK01+35, 10014, 99, 0, -1, 0, -100, 255); -- ズドドッ
setEffShake(SP_ATK01+35, ct, 81, 20);
setEffScaleKey( SP_ATK01+35, ct, 2.4, 2.4);
setEffRotateKey(SP_ATK01+35, ct, 70);
setEffAlphaKey(SP_ATK01+35, ct, 255);
setEffAlphaKey(SP_ATK01+70+diff01+35, ct, 255);
setEffAlphaKey(SP_ATK01+92+diff01+35, ct, 0);
...
setMoveKey(  SP_dodge+6,  0, -50,  0,   0);
