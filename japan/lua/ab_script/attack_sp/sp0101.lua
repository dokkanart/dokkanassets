...
setEffScaleKey(431, kamehame_beam2, 1, 1);
setEffScaleKey(433, kamehame_beam2, 3.5, 3.5);
setShake( 370, 63, 13);
setShake( 433, 38, 21);
...
setMoveKey(tyo2+305,    0,   -600,  0,   85);
--setMoveKey(tyo2+308,    0,   -1100,  0,   0);
...
setEffScaleKey(tyo2+260, 1.3, 1.3);
...
setEffScaleKey(370, kamehame_beam2, 1, 1);
setEffScaleKey(373, kamehame_beam2, 2.5, 2.5);

entryFade(375, 15,  17, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
setShake( tyo2+260, 40, 13);
setShake( 373, 10, 21);
...
SP_dodge = 185+174; --エンドフェイズのフレーム数を置き換える
...
setMoveKey(tyo2-6+470,    1,    0,   0,   128);
setScaleKey(tyo2-6+470,    1,  0.1, 0.1);

changeAnime(tyo2-6+470, 1, 107);                         -- 手前ダメージ
entryEffect(tyo2-6+478, 1509,  0x80,  -1,  0,  0,  0);   -- 爆発
playSe(tyo2-6+478, SE_10);

setMoveKey(tyo2-6+478,   1,    0,   0,   128);
setMoveKey(tyo2-6+485,   1,  -60,  -200,  -100);
--setMoveKey(tyo2-6+486,   1,  -60,  -200,  -100);
setDamage(tyo2-6+486, 1, 0);  -- ダメージ振動等
setShake(tyo2-6+477,21,15);
setShake(tyo2-6+483,39,10);

setRotateKey(tyo2-6+470,  1,  30 );
setRotateKey(tyo2-6+472,  1,  80 );
setRotateKey(tyo2-6+474,  1, 120 );
setRotateKey(tyo2-6+476,  1, 160 );
setRotateKey(tyo2-6+478,  1, 200 );
setRotateKey(tyo2-6+480,  1, 260 );
setRotateKey(tyo2-6+482,  1, 320 );
setRotateKey(tyo2-6+484,  1,   0 );

setShakeChara(tyo2-6+485, 1, 5,  10);
setShakeChara(tyo2-6+490, 1, 10, 20);

-- 書き文字エントリー
ct = entryEffectLife(tyo2-6+485, 10005, 100, 0, -1, 0, -50, 230); -- ガッ
setEffShake(tyo2-6+485, ct, 30, 10);
setEffRotateKey(tyo2-6+485, ct, -40);
setEffScaleKey(tyo2-6+485, ct, 4.0, 4.0);
setEffScaleKey(tyo2-6+486, ct, 2.0, 2.0);
setEffScaleKey(tyo2-6+487, ct, 2.6, 2.6);
setEffScaleKey(tyo2-6+488, ct, 4.0, 4.0);
setEffScaleKey(tyo2-6+489, ct, 2.6, 2.6);
setEffScaleKey(tyo2-6+490, ct, 3.8, 3.8);
setEffScaleKey(tyo2-6+580, ct, 3.8, 3.8);
setEffAlphaKey(tyo2-6+485, ct, 255);
setEffAlphaKey(tyo2-6+575, ct, 255);
setEffAlphaKey(tyo2-6+585, ct, 0);

playSe(tyo2-6+473, SE_11);
shuchusen = entryEffectLife(tyo2-6+473, 906, 120, 0x00,  -1, 0,  0,  0);   -- 集中線
entryEffect(tyo2-6+473, 1600,  0x00,  -1,  0,30, -130);   -- ひび割れ

-- ダメージ表示
dealDamage(tyo2-6+486);

--entryFade(tyo2-6+570, 9,  10, 1, 8, 8, 8, 255);             -- black fade
