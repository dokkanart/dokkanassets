...
playSe( 30, SE_01);
entryFadeBg( 30, 5, 220, 5, 255, 255, 255, 255);       -- ベース青　背景
...
spname = entryEffectLife( 260, 1508, 88, 0,  -1,  0,  0,  0);               -- かめはめ波文字
setEffReplaceTexture(spname, 7, 4);                                  -- 技名テクスチャ差し替え

--背景を暗くする
entryFadeBg( 260, 0, 89, 0, 10, 10, 10, 255);       -- ベース暗め　背景
...
entryEffectLife( 320, 920, 28, 0x80,  -1,  0,  0,  0); -- 流線
...
SP_dodge = 325; --エンドフェイズのフレーム数を置き換える
...
setMoveKey(  325,    0,      -100,  0,   0);
setMoveKey(  331,    0,      0,  0,   0);

--けられた相手が飛んでいく
setMoveKey(   332,   1,      150,  0,   0);
setMoveKey(  338,    1,      700,  200,   0);



changeAnime( 331, 0, 31);-- 蹴り
playSe( 331,1010);
--蹴りエフェクト
entryEffect( 331,  6,  0,  -1,  0,  150, 0);--HIT
entryEffect( 331,  906,  0,  -1,  0,  150, 0);
--文字
entryEffect(  331,  908,  0,  -1,  0,  150,  0);
entryKakimoji(  331,  19,  4,  -1,  0,  -25,  -250);--どん

--Eやられ
changeAnime( 332, 1,  106);--ダメージ
setDamage( 332,  1,  0);--ダメージ振動等
...
setDisp( 349, 0, 0);
setDisp( 349, 1, 1);
setMoveKey(  349,    0,      0,  0,   0);
setMoveKey(  349,    1,      700,  0,   0);
setScaleKey( 349,    1,  2.5, 2.5);
setMoveKey(  350,    1,    0,   0,   128);
setScaleKey( 350,    1,  0.1, 0.1);
setMoveKey(  350,    0,      700,  0,   0);

changeAnime( 350, 1, 107);                         -- 手前ダメージ
entryEffect( 358, 1509,  0x80,  -1,  0,  0,  0);   -- 爆発
playSe( 358, SE_10);

setMoveKey(  358,   1,    0,   0,   128);
setMoveKey(  365,   1,  -60,  -200,  -100);
--setMoveKey(  366,   1,  -60,  -200,  -100);
setDamage( 366, 1, 0);  -- ダメージ振動等
setShake(357,6,15);
setShake(363,15,10);

setRotateKey( 350,  1,  30 );
setRotateKey( 352,  1,  80 );
setRotateKey( 354,  1, 120 );
setRotateKey( 356,  1, 160 );
setRotateKey( 358,  1, 200 );
setRotateKey( 360,  1, 260 );
setRotateKey( 362,  1, 320 );
setRotateKey( 364,  1,   0 );

setShakeChara( 365, 1, 5,  10);
setShakeChara( 370, 1, 10, 20);

-- 書き文字エントリー
ct = entryEffectLife( 365, 10005, 100, 0, -1, 0, -50, 230); -- ガッ
setEffShake(365, ct, 30, 10);
setEffRotateKey( 365, ct, -40);
setEffScaleKey( 365, ct, 4.0, 4.0);
setEffScaleKey( 366, ct, 2.0, 2.0);
setEffScaleKey( 367, ct, 2.6, 2.6);
setEffScaleKey( 368, ct, 4.0, 4.0);
setEffScaleKey( 369, ct, 2.6, 2.6);
setEffScaleKey( 370, ct, 3.8, 3.8);
setEffScaleKey( 460, ct, 3.8, 3.8);
setEffAlphaKey( 365, ct, 255);
setEffAlphaKey( 455, ct, 255);
setEffAlphaKey( 465, ct, 0);

playSe( 353, SE_11);
shuchusen = entryEffectLife( 353, 906, 120, 0x00,  -1, 0,  0,  0);   -- 集中線
entryEffect( 353, 1600,  0x00,  -1,  0,30, -130);   -- ひび割れ

-- ダメージ表示
dealDamage(366);

entryFade( 450, 9,  10, 1, 8, 8, 8, 255);             -- black fade
...
--entryEffectLife( SP_ATK_0,   1500,  55, 0,     -1,  0,  0,  0);   -- eff_003 (気)
entryFadeBg( SP_ATK_0, 10, 45, 5, 255, 255, 255, 255);       -- ベース白　背景
...
changeAnime( SP_ATK_1+36, 0, 17);                       -- 溜め!

--画面明けには暗くしておく
entryFadeBg( SP_ATK_1+50, 0, 105, 0, 10, 10, 10, 255);       -- ベース暗め　背景
...
shun = entryEffect(  SP_ATK_2+50,  SP_02,  0,  0,  0,  -100,  100);
...
setMoveKey(  SP_ATK_2+55,    0,      -100,  0,   0);

setDisp ( SP_ATK_2+56, 0, 1);


entryEffectLife( SP_ATK_2+60, 920, 28, 0x80,  -1,  0,  0,  0); -- 流線
...
SP_dodge = 141; --エンドフェイズのフレーム数を置き換える
...
setScaleKey( SP_ATK_2+99,   0, 0.65, 0.65);

setMoveKey(  SP_ATK_2+65,    0,      -100,  0,   0);
setMoveKey(  SP_ATK_2+71,    0,      0,  0,   0);

--けられた相手が飛んでいく
setMoveKey(   SP_ATK_2+72,   1,      150,  0,   0);
setMoveKey(  SP_ATK_2+76,    1,      700,  200,   0);


changeAnime( SP_ATK_2+71, 0, 31);                                    -- 蹴りポーズ
playSe( 1010);
--蹴りエフェクト
entryEffect( SP_ATK_2+71,  6,  0,  1,  0,  -30, 0);--HIT
entryEffect( SP_ATK_2+71,  906,  0,  1,  0,  150, 0);
--文字
entryEffect(  SP_ATK_2+71,  908,  0,  -1,  0,  150,  0);
entryKakimoji(  SP_ATK_2+71,  19,  4,  -1,  0,  -25,  -250);--どん
--Eやられ
changeAnime( SP_ATK_2+72,  1,  106);--ダメージ
setDamage( SP_ATK_2+72,  1,  0);--ダメージ振動等



--amehame_beam = entryEffectLife( SP_ATK_2+72, SP_03, 56, 0x40,  0,  300,  100,  0);   -- 伸びるかめはめ波
...
setMoveKey(  SP_ATK_4-1,    1,      700,  200,   0);
...
setScaleKey( SP_ATK_4,    1,  0.68, 0.68);
setMoveKey(  SP_ATK_4-1,    0,      0,  0,   0);
