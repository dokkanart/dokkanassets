...
kame_hand = entryEffect( 70, SP_01, 0x40, 0, 300, -20, 90);   -- 手のカメハメ波部
setEffScaleKey( 70, kame_hand, 0.48, 0.48);
...
kamehame_beam = entryEffectLife( 260, SP_02, 109, 0x40,  0,  300, 60,  0);   -- 伸びるかめはめ波
...
entryEffectLife( SP_ATK_0, 1503, 20, 0x80, -1, 0, 0, 0);   -- eff_004 (気)
entryEffectLife( SP_ATK_0, 1502, 20, 0, -1, 0, 0, 0);   -- eff_003 (気)

kame_hand0 = entryEffectLife( 6, SP_01, 20, 0x40, 0, 300, -20, 90);   -- 手のカメハメ波部
setEffScaleKey( 6, kame_hand0, 0.48, 0.48);

-- ** エフェクト等 ** --
aura = entryEffectLife( SP_ATK_0, 311, 20, 0x40+0x80, 0, 1, 0, 0); -- オーラ
...
kamehame_beam = entryEffectLife( SP_ATK_2+1, SP_02, 90, 0x40,  0,  300,  60,  0);   -- 伸びるかめはめ波

playSe(SP_ATK_2+1, SE_07);

--spname = entryEffect( SP_ATK_2, 1508,  0x00,  -1,  0,  0,  0);               -- かめはめ波文字
--setEffReplaceTexture(spname, 7, 4);                                  -- 技名テクスチャ差し替え
...
entryFade( SP_ATK_2-3, 3, 2, 5, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
entryEffectLife( SP_ATK_2+2, 920, 89, 0x80,  -1,  0,  0,  0); -- 流線
