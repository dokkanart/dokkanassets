...
kamehame_beam = entryEffectLife( 260, SP_02, 109, 0x40,  0,  300,  300,  205);   -- 伸びるかめはめ波
...
eff01 = entryEffect( SP_ATK_0,   1503,   0x80,  -1,  0,  0,  0);   -- eff_004 (気)
--entryFadeBg( 0, 2, 40, 10, 0, 0, 0, 255);       -- ベース暗め　背景
eff02 = entryEffect( SP_ATK_0,   1502,   0,     -1,  0,  0,  0);   -- eff_003 (気)
setEffAlphaKey( SP_ATK_0, eff01, 255);
setEffAlphaKey( SP_ATK_0, eff02, 255);

--kame_hand = entryEffect( 0, SP_01, 0x40+0x80, 0,  300,  0,  0);   -- 手のカメハメ波部
--setEffScaleKey( SP_ATK_0, kame_hand, 0.5, 0.5);
--setEffAlphaKey( SP_ATK_0,kame_hand, 255);
...
--setEffAlphaKey( SP_ATK_1,kame_hand, 255);
--setEffAlphaKey( SP_ATK_1+3,kame_hand, 0);
...
-- ** カード消し ** --
setEffAlphaKey( (SP_ATK_1), speff, 255);
setEffAlphaKey( (SP_ATK_1+50), speff, 255);
setEffAlphaKey( (SP_ATK_2+2), speff, 0);
setEffAlphaKey( (SP_ATK_0), aura, 255);
...
kamehame_beam_st = entryEffectLife( SP_ATK_2, SP_02, 6, 0x40,  0,  300,  300,  205);   -- 伸びるかめはめ波
kamehame_beam = entryEffectLife( SP_ATK_2, SP_02, 90, 0x40,  0,  300,  300,  205);   -- 伸びるかめはめ波
setEffAlphaKey( (SP_ATK_2), kamehame_beam, 0);
setEffAlphaKey( (SP_ATK_2+6), kamehame_beam, 255);


for cnt = 0, 90, 3 do
playSe( SP_ATK_2+cnt, SE_13);
end
...
-- 途中からカメハメハが伸びる
setMoveKey(  SP_ATK_2+89,    0,   -100,  0,   0);
setMoveKey(  SP_ATK_2+90,    0,   -100,  0,   0);
setMoveKey(  SP_ATK_2+100,    0,   -1100,  0,   0);

entryFade( SP_ATK_2+5, 3, 2, 5, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade

-- ** エフェクト等 ** --
entryFadeBg( SP_ATK_2, 0, 109, 0, 10, 10, 10, 180);       -- ベース暗め　背景
entryFadeBg( SP_ATK_2, 0, 40, 0, 0, 0, 0, 255);    -- 最初だけ明るい　背景
entryFadeBg( SP_ATK_2+40, 0, 2, 8, 100, 140, 250, 250);     -- 青い　背景

entryEffectLife( SP_ATK_2+5, 920, 90, 0x80,  -1,  0,  0,  0); -- 流線
...
-- かめはめは迫る(100F)
------------------------------------------------------
kamehame_beam3 = entryEffectLife( SP_ATK_3, SP_03,   70, 0, -1,  0,  0,  0);   -- 迫るかめはめ波
for cnt = 0, 70, 3 do
playSe( SP_ATK_3+cnt, SE_13);
end
...
-- 敵吹っ飛ぶモーション
changeAnime( 229, 1, 108);
entryEffectLife( 229, 10016, 10, 0, 0, 0, 0, 0); --
entryEffectLife( 230, 906, 30, 0x00,  -1, 0,  0,  0);   -- 集中線
setMoveKey(  229, 1,  120,    0,  0);
--entryFlash( 229, 2, 0, 0, 0, 200);
setMoveKey(  235, 1,  300,    0,  128);
setScaleKey( 233, 1,  1.6, 1.6);
setScaleKey( 235, 1,  1, 1);

-- ** エフェクト等 ** --
setShakeChara( SP_ATK_3+20+5, 1, 99, 20);

entryFadeBg( SP_ATK_3, 0, 72, 4, 10, 10, 10, 150);          -- ベース暗め　背景

entryEffectLife( SP_ATK_3, 921, 72, 0x80,  -1,  0,  0,  0); -- 流線斜め
...
entryFade( 238, 10,  3, 10, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
playSe( 229, SE_09);
...
SP_dodge = 185+40; --エンドフェイズのフレーム数を置き換える
