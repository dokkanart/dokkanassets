...
kamehame_beam_st = entryEffectLife( SP_ATK_2+1, SP_02, 4, 0x40,  0,  300,  400,  250);   -- 伸びるかめはめ波
kamehame_beam = entryEffectLife( SP_ATK_2+1, SP_02, 90, 0x40,  0,  300,  0,  0);   -- 
setEffAlphaKey( (SP_ATK_2+1),  kamehame_beam, 0);
setEffAlphaKey( (SP_ATK_2+5),  kamehame_beam, 255);

playSe(SP_ATK_2+1, SE_07);

spname = entryEffectLife( SP_ATK_2, 1508,  92, 0x00,  -1,  0,  0,  0);               -- かめはめ波文字
setEffReplaceTexture(spname, 7, 4);                                  -- 技名テクスチャ差し替え

-- 途中からカメハメハが伸びる
setMoveKey(  SP_ATK_2+21,    0,   -100,  0,   0);
setMoveKey(  SP_ATK_2+22,    0,   -100,  0,   0);
setMoveKey(  SP_ATK_2+33,    0,   -1300,  0,   0);
setDisp( SP_ATK_2+33, 0, 0);

setMoveKey(  SP_ATK_2+75,    0,   -1300,  0,   0);
setMoveKey(  SP_ATK_2+80,    0,   -1200,  0,   -50);
...
setShakeChara( SP_ATK_2+5, 0, 27, 15);
