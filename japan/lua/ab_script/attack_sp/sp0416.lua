...
spep1=0;
entryFadeBg( spep1+20, 20, 40, 0, 0, 0, 0, 210);       -- ベース暗め　背景
--気を貯める
...
playSe( spep1+5, SE_01);

setDisp(spep1,0,0);

-- ** エフェクト等 ** --
Tame = entryEffect( spep1, SP_01, 0x100,      -1,  0,  0,  0);   -- 手のカメハメ波部
setEffScaleKey( spep1, Tame, 1.0, 1.0);
setEffMoveKey( spep1, Tame, 0, 0,0);

entryFade( spep1+52, 3, 6, 3, fcolor_r, fcolor_g, fcolor_b,  255);     -- white fade

-- 書き文字エントリー
ctZuzun = entryEffectLife( spep1+20, 10013, 40, 0x100, -1, 0, 0, 250,0); -- ズズンッ

setEffMoveKey( spep1+20,ctZuzun,0,250,0);
setEffMoveKey( spep1+37,ctZuzun,0,350,0);
setEffMoveKey( spep1+69,ctZuzun,0,350,0);

setEffRotateKey( spep1+20,ctZuzun,-10);
setEffShake(spep1+21, ctZuzun, 40, 20);
setEffAlphaKey(spep1+20, ctZuzun, 255);
setEffAlphaKey(spep1+50, ctZuzun, 255);
setEffAlphaKey(spep1+70, ctZuzun, 0);
setEffScaleKey( spep1+20, ctZuzun, 0.2, 0.2);
setEffScaleKey( spep1+25, ctZuzun, 3.0, 3.0);
playSe( spep1+20, SE_02);

shuchusen = entryEffectLife( spep1+20, 906, 40, 0x100,  -1, 0,  0,  0);   -- 集中線
setEffScaleKey( spep1, shuchusen, 1.0, 1.0);
setEffScaleKey( spep1+40, shuchusen, 1.0, 1.0);
...
-- ** エフェクト等 ** --
entryFadeBg( spep2, 0, 99, 0, 0, 0, 0, 210);       -- ベース暗め　背景
...
Hanatsu= entryEffectLife( spep4, SP_03, 110, 0x100,  0,  -1,  0,  0);   -- 伸びるかめはめ波
...
entryFadeBg( spep7, 0, 120, 0,0, 0, 0, 180);          -- ベース暗め　背景

entryEffectLife( spep7, 921, 90, 0x80,  -1,  0,  0,  0); -- 流線斜め

SP_dodge = 400; --エンドフェイズのフレーム数を置き換える　（spep7+43）
...
-- ** エフェクト等 ** --
Tame = entryEffect( spep1+0, SP_01e, 0x100,      -1,  0,  0,  0);   -- 手のカメハメ波部
setEffScaleKey( spep1+0, Tame, 1.0, 1.0);
setEffMoveKey( spep1+0, Tame, 0, 50,0);
...
Hanatsu= entryEffectLife( spep4, SP_03e, 110, 0x100,  0,  -1,  0,  0);   -- 伸びるかめはめ波
setEffMoveKey( spep4, Hanatsu, 0, 0);
setEffMoveKey( spep4+110, Hanatsu, 0, 0, 0);
