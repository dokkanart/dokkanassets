...
entryFade( 250, 6, 6, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
--]]
setMoveKey(  259,    0,      0,  0,   0);
setMoveKey(  260,    0,      -600,  0,   50);
setMoveKey(  270,    0,      -150,  0,   0);
setMoveKey(  290,    0,      -150,  0,   0);
...
setMoveKey(  300,    0,   -350,  0,   50);
setMoveKey(  305,    0,   -500,  0,   65);
--setMoveKey(  308,    0,   -1100,  0,   0);
setDisp( 305, 0, 0);
...
entryFadeBg( 300, 0, 10, 8, 0, 0, 0, 250);     -- 青い　背景
entryFadeBg( 310, 0, 55, 0, 0, 0, 0, 255);       -- ベース暗め　背景

sen2 = entryEffectLife( 260, 921, 39, 0x80,  -1,  0,  0,  0); -- 流線
setEffRotateKey(260, sen2, 190);
setShakeChara( 270, 0, 24, 50);
setEffScaleKey(  260, sen2, 1.6, 1.6);

shuchusen = entryEffectLife( 260, 906, 109, 0x80,  -1, 0,  0,  0);   -- 集中線
...
SP_dodge = 411; --エンドフェイズのフレーム数を置き換える
...
setEffScaleKey(469,explosion,1.1,1.1);
setEffScaleKey(580,explosion,1.1,1.1);
setDisp( 469, 1, 1);
...
-- ** エフェクト等 ** --
aura = entryEffectLife( tyo+70,   311, 99, 0x40,  0,  1,  0,  0); -- オーラ
setEffScaleKey( tyo+30, aura, 1.5, 1.5);
...
print ("_SPECIAL_SKILL_LEVEL_ == 1");
playSe( tyo2+170, SE_05);
speff = entryEffect( tyo2+170, 1120, 0, -1, 0, 0, 0); -- カード
setEffReplaceTexture( speff, 1, 1);
setEffReplaceTexture( speff, 2, 0); -- カード差し替え
setEffReplaceTexture( speff, 5, 4); -- 技名テクスチャ差し替え
...
print ("_SPECIAL_SKILL_LEVEL_ == 2");
playSe( tyo2+170, SE_05);
speff = entryEffect( tyo2+170, 1121, 0, -1, 0, 0, 0); -- カード
setEffReplaceTexture( speff, 1, 1);
setEffReplaceTexture( speff, 2, 0); -- カード差し替え
setEffReplaceTexture( speff, 5, 4); -- 技名テクスチャ差し替え
end

entryFade(tyo2+250, 6, 6, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
setMoveKey(  tyo2+259,    0,      0,  0,   0);
setMoveKey(  tyo2+260,    0,      -600,  0,   50);
setMoveKey(  tyo2+270,    0,      -150,  0,   0);
setMoveKey(  tyo2+290,    0,      -150,  0,   0);
...
setMoveKey(tyo2+300,    0,   -350,  0,   50);
setMoveKey(tyo2+310,    0,   -500,  0,   65);
--setMoveKey(tyo2+308,    0,   -1100,  0,   0);
setDisp( tyo2+310, 0, 0);
...
entryFadeBg(tyo2+300, 0, 10, 8, 0, 0, 0, 250);     -- 青い　背景
entryFadeBg(tyo2+310, 0, 60, 0,0, 0, 0, 255);       -- ベース暗め　背景

sen2 = entryEffectLife(tyo2+260, 921, 109, 0x80,  -1,  0,  0,  0); -- 流線
setEffRotateKey(tyo2+260, sen2, 180);
setShakeChara(tyo2+270, 0, 54, 50);
setEffScaleKey(  tyo2+260, sen2, 1.6, 1.6);
...
SP_dodge = 185+188; --エンドフェイズのフレーム数を置き換える
...
setEffScaleKey(tyo2+580,bom,1.1,1.1);
