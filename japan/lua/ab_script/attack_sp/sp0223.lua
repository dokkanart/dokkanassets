...
SP_01 = 108031; --ゴーストが10体出現既存から変更無し
SP_02 = 108036; --ゴースト8体が突撃
SP_03 = 108037; --ゴースト8体突撃時に敵の前面に表示する爆発エフェクト郡
SP_04 = 108038; --ゴースト8体突撃時に敵の背面に表示する突撃ゴースト郡
--SP_05 = 1571; --大爆発3
SP_05 = 108035; --2体のゴーストが張り切りすぎて爆発
SP_06 = 109054;--敵が光弾を避ける時の瞬間移動エフェクト
...
entryFadeBg( 0, 0, 239, 0, 10, 10, 10, 180);       -- ベース暗め　背景
...
-- ** エフェクト等 ** 
aura = entryEffectLife(  50,   311, 179, 0x40,  0,  1,  0,  -50); -- オーラ
setShakeChara( 50, 0, 49, 5);

kame_hand = entryEffect( 50, SP_01, 0x40+0x100,      0,  300,  0,  50);   -- ゴースト出現
...
kame_hand2 = entryEffect( spep_1, SP_05, 0x40+0x100,      0,  300,  0,  50);   -- ゴースト爆発



playSe( spep_1+50, SE_09);

entryFade( spep_1+65, 3, 2, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
setScaleKey(  spep_1+69,   0, 1.0, 1.0);
setEffScaleKey( spep_1+69, kame_hand, 1.0, 1.0);
--setMoveKey(   spep_1+69,   0,    -30, 0,   0);
...
setMoveKey(  spep_4,    0,  150,  -50,   0);
setMoveKey(  spep_4+63,    1,  150,  -50,   0);

changeAnime( spep_4+120, 1, 106);                                    -- ダメージ
setShake(spep_4+120,70, 20);

syunkan1=entryEffect( spep_4+42, SP_06, 0x40+0x100,  1, 300,  0, 0); --瞬間移動
setDisp( spep_4+42, 1, 0);

syunkan2=entryEffect( spep_4+ 65, SP_06, 0x40+0x100,  1, 300,  0, 0); --瞬間移動
setMoveKey(  spep_4+64,    1,  200,  50,   0);
setDisp( spep_4+70, 1, 1);

setMoveKey(  spep_4+65,    1,   -250,  350,   80);
setMoveKey(  spep_4+120,    1,   150,  -100,   0);

bakuhatu=entryEffect( spep_4+24, SP_03, 0x40+0x100,  0, 300,  -150, 100); --爆発
--playSe( spep_4+22, SE_09);
playSe( spep_4+60, SE_09);
--playSe( spep_4+90, SE_09);
playSe( spep_4+124, SE_09);
playSe( spep_4+130, SE_09);
playSe( spep_4+150, SE_09);

-- ** エフェクト等 ** --
entryFadeBg( spep_4, 0, 168, 0, 10, 10, 10, 180);       -- ベース暗め　背景

sen2 = entryEffectLife( spep_4, 921, 168, 0x80,  -1,  0,  0,  0); -- 流線
setEffRotateKey( spep_4, sen2, -110);
setShakeChara( spep_4+120, 1, 54, 50);
setEffScaleKey( spep_4, sen2, 1.6, 1.6);

kamehame_beam=entryEffect( spep_4+15, SP_04, 0x40+0x100,  0, 300,  -80, 50); --ゴースト：敵が動くため透明な味方キャラに紐付け
setEffScaleKey( spep_4+15, kamehame_beam, 1.0, 1.0);


entryFade( spep_4+102+60, 5,  5, 5, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
setScaleKey( spep_4+108+60,   1, 1.6, 1.6);
setMoveKey(  spep_4+108+60,    1,   150,  -50,   0);

removeAllEffect( spep_4+102+66 );
setMoveKey(  spep_4+102,    0,  150,  -50,   0);

spep_5=spep_4+102+10+60; --410 469
...
SP_dodge = 559; --エンドフェイズのフレーム数を置き換える
...
setScaleKey( spep_5,   1, 1.8, 1.8);
setDisp(  spep_5, 1, 1);
setMoveKey(   spep_5,    1,  120,    0,  128);
...
playSe(  spep_5+1, SE_10);

setMoveKey(   spep_5+8,    1,    0,   0,   128);
setMoveKey(   spep_5+15,   1,  -60,  -200,  -100);
--setShake( spep_5+10,6,15);
--setShake( spep_5+17,15,9);
...
setShakeChara(  spep_5+14, 1, 5,  10);
setShakeChara(  spep_5+20, 1, 10, 20);

-- 書き文字エントリー
ct = entryEffectLife(  spep_5+14, 10005, 120, 0, -1, 0, -50, 230); -- ガッ
...
setEffAlphaKey(  spep_5+120, ct, 255);

playSe(  spep_5+3, SE_11);
shuchusen = entryEffectLife(  spep_5, 906, 120, 0x00,  -1, 0,  0,  0);   -- 集中線
entryEffect(  spep_5+3, 1600,  0x00,  -1,  0,30, -130);   -- ひび割れ

-- ダメージ表示
dealDamage( spep_5+15 -10);
