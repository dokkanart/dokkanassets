...
entryFade( 165 -2, 3, 2 +2, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
entryFade( 255, 3, 2 +2, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
kamehame_beam = entryEffectLife( 260, SP_02, 109, 0x40,  0,  300,  130,  0);   -- 伸びるかめはめ波
...
entryEffect( SP_ATK_0,   1503,   0x80,  -1,  0,  0,  0);   -- eff_004 (気)(6F)
entryEffect( SP_ATK_0,   1502,   0,     -1,  0,  0,  0);   -- eff_003 (気)(6F)

kame_hand = entryEffect( 0, SP_01, 0x40, 0,  300,  0,  0);   -- 手のカメハメ波部
setEffScaleKey( SP_ATK_0, kame_hand, 0.5, 0.5); -- (6F)

-- ** エフェクト等 ** --
aura = entryEffectLife( SP_ATK_0,   311, 100, 0x40,  0,  1,  0,  0); -- オーラ(6F)
setEffScaleKey( SP_ATK_0, aura, 1.5, 1.5); -- (6F)
setShakeChara( SP_ATK_0, 0, 49, 5); -- (6F)
--entryFade( SP_ATK_1, 3, 2, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade(16F)

------------------------------------------------------
-- カードカットイン
------------------------------------------------------

speff = entryEffect(  SP_ATK_1+3,   1507,   0,  -1,  0,  0,  0);   -- カード(16F)
setEffReplaceTexture( speff, 1, 1);
setEffReplaceTexture( speff, 2, 0);                         -- カード差し替え
--setEffReplaceTexture( speff, 5, 4);                                  -- 技名テクスチャ差し替え
playSe( SP_ATK_1+6, SE_12);

spname = entryEffect( SP_ATK_1+4, 1508,  0,  -1,  0,  0,  0);               -- かめはめ波文字
setEffReplaceTexture(spname, 7, 4);                                  -- 技名テクスチャ差し替え
...
setEffAlphaKey( (SP_ATK_1+20), speff, 255);--カードから16F後
setEffAlphaKey( (SP_ATK_1+70), speff, 0); -- 上記から28F後
...
kamehame_beam = entryEffectLife( SP_ATK_2+1, SP_02, 90, 0x40,  0,  300,  130,  0);   -- 伸びるかめはめ波
...
SP_dodge = 185+5; --エンドフェイズのフレーム数を置き換える
