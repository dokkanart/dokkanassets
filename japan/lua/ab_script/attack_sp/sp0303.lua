...
entryEffectLife(  0,   1503, 140,  0x80,  -1,  0,  0,  0);   -- eff_004 (気)
entryEffectLife(  0,   1502, 140,  0,     -1,  0,  0,  0);   -- eff_003 (気)

setMoveKey(   0,   0,  20, -54,   0);
setMoveKey(   117,   0,  20, -54,   0);
...
-- ** エフェクト等 ** --
aura = entryEffectLife(  0,   311, 117, 0x40+0x100,  0,  300,  0,  -200); -- オーラ
setEffMoveKey(   0,   aura,    0,  -200,   0);
setEffMoveKey(  117,   aura,    0,  -200,  0);
--setEffScaleKey( 0, aura, 1, 1);
--setEffScaleKey( 40, aura, 1, 1);
setEffScaleKey( 0, aura, 1.5, 1.5);

kame_hand = entryEffect( 0, SP_01, 0x40+0x100, 0,  300,  -25,  20);   -- 手のカメハメ波部 50end
...
-- 書き文字エントリー
ct = entryEffectLife( 30, 10008, 87, 0x100, -1, 0, -190 -20, 300 -20);    -- ゴゴゴ・・・
...
kamehame_beam_st = entryEffectLife( spep_3, SP_02, 44, 0x40+0x80,  0,  300,  150,  150);   -- 伸びるかめはめ波oku

kamehame_beam = entryEffectLife( spep_3, SP_02, 60, 0x40+0x100,  0,  300,   150,  150);   -- 伸びるかめはめ波temae
...
setEffRotateKey(spep_4, kamehame_beam2, -50);
setEffAlphaKey(spep_4, kamehame_beam2, 255);
setEffAlphaKey(spep_4+45, kamehame_beam2, 255);
setEffAlphaKey(spep_4+50, kamehame_beam2, 0);
...
kamehame_beam5 = entryEffect( spep_4+35, 190000,   0x80, -1,  0,  0,  0);   -- 迫るかめはめ波
...
entryFade( spep_4+92-10, 5, 15, 6, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
...
SP_dodge = 300; --エンドフェイズのフレーム数を置き換える
...
kame_hand = entryEffect( 0, SP_01, 0x40+0x80, 0,  300,  -25,  20);   -- 手のカメハメ波部 50end
setEffScaleKey( 0, kame_hand, 0.5, 0.5);
