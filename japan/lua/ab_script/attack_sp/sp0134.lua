...
setScaleKey(   0,   0, 1.45, 1.45);
setScaleKey(   1,   0, 1.45, 1.45);
...
kame_hand = entryEffect( 70, SP_01, 0x40+0x100,      0,  0,  -20,  165);   -- 手のカメハメ波部
--kame_hand2 = entryEffect( 70, SP_01, 0x40,      0,  300,  -100,  100);   -- 手のカメハメ波部

setEffScaleKey( 70, kame_hand, 0.46, 0.46);
--setEffScaleKey( 70, kame_hand2, 0.5, 0.5);

entryFade( 165, 3, 2, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade

-- ** エフェクト等 ** --
aura = entryEffectLife(  70,   311, 99, 0x40,  0,  1,  -20,  0); -- オーラ
...
-- 途中からカメハメハが伸びる
setMoveKey(  299,    0,   -200,  0,   0);
setMoveKey(  300,    0,   -200,  0,   0);
...
-- 書き文字エントリー
ct = entryEffectLife( 370, 10014, 99, 0, -1, 0, 0, 325); -- ズドドッ
...
aura1 = entryEffect( SP_ATK_0,   1503,   0x80,  -1,  0,  0,  0);   -- eff_004 (気)
aura2 = entryEffect( SP_ATK_0,   1502,   0,     -1,  0,  0,  0);   -- eff_003 (気)

setEffAlphaKey(SP_ATK_0, aura1, 255);
setEffAlphaKey(SP_ATK_0, aura2, 255);
setEffScaleKey(SP_ATK_0, aura1, 1.0, 1.0);
setEffScaleKey(SP_ATK_0, aura2, 1.0, 1.0);


-- ** エフェクト等 ** --
aura = entryEffectLife( SP_ATK_0,   311, 100, 0x40,  0,  1,  -20,  0); -- オーラ
setEffScaleKey( SP_ATK_0, aura, 1.4, 1.4);
setEffScaleKey( SP_ATK_2-1, aura, 1.4, 1.4);
setEffScaleKey( SP_ATK_2, aura, 2.5, 2.5);
setShakeChara( SP_ATK_0+40, 0, 49, 5);
setEffAlphaKey(SP_ATK_0, aura, 155);

kame_hand = entryEffectLife( SP_ATK_0, SP_01, 72, 0x40+0x100, 0,  0,  -20,  165);   -- 手のカメハメ波部
setEffScaleKey( SP_ATK_0, kame_hand, 0.46, 0.46);
...
print ("_SPECIAL_SKILL_LEVEL_ == 1");
playSe( SP_ATK_1 +22, SE_12);
speff = entryEffect( SP_ATK_1, 1120, 0, -1, 0, 0, 0); -- カード
setEffReplaceTexture( speff, 1, 1);
setEffReplaceTexture( speff, 2, 0); -- カード差し替え
setEffReplaceTexture( speff, 5, 4); -- 技名テクスチャ差し替え
...
print ("_SPECIAL_SKILL_LEVEL_ == 2");
playSe( SP_ATK_1 + 22, SE_12 );
speff = entryEffect( SP_ATK_1, 1121, 0, -1, 0, 0, 0); -- カード
setEffReplaceTexture( speff, 1, 1);
setEffReplaceTexture( speff, 2, 0); -- カード差し替え
setEffReplaceTexture( speff, 5, 4); -- 技名テクスチャ差し替え
end
...
setScaleKey( SP_ATK_2-1,   0, 1.46, 1.46);
setScaleKey( SP_ATK_2,   0, 0.7, 0.7);

setMoveKey(  SP_ATK_2+5,    0,      0,  0,   0);
setMoveKey(  SP_ATK_2+10,   0,   -200,  0,   0);
...
-- 途中からカメハメハが伸びる
setMoveKey(  SP_ATK_2+21,    0,   -200,  0,   0);
setMoveKey(  SP_ATK_2+22,    0,   -200,  0,   0);
...
setEffScaleKey(SP_ATK_3+82, kamehame_beam2, 1, 1);
setEffScaleKey(SP_ATK_3+84, kamehame_beam2, 2.5, 2.5);
setDamage( SP_ATK_3+85, 1, 0);  -- ダメージ振動等
...
setScaleKey( SP_ATK_3,   1,   1.6,  1.6);
...
-- ** エフェクト等 ** --
setShakeChara( SP_ATK_3, 1, 99, 20);

entryFadeBg( SP_ATK_3, 0, 94, 4, 10, 10, 10, 150);          -- ベース暗め　背景

entryEffectLife( SP_ATK_3, 921, 99, 0x80,  -1,  0,  0,  0); -- 流線斜め
...
-- 書き文字エントリー
ct = entryEffectLife( SP_ATK_3, 10014, 99, 0, -1, 0, 50, 325); -- ズドドッ
...
setEffAlphaKey(SP_ATK_3+60, ct, 255);
setEffAlphaKey(SP_ATK_3+80, ct, 0);


entryFade( SP_ATK_3+92, 6,  7, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
--entryFade( 462, 6,  7, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade
playSe( SP_ATK_3+85, SE_09);

setDisp( SP_ATK_4, 1, 1);
setMoveKey(  SP_ATK_3,    1,  0,    0,  0);
setMoveKey(  SP_ATK_4,    1,  0,    0,  0);
setScaleKey( SP_ATK_4,    1,  1.6, 1.6);
...
SP_dodge = 192; --エンドフェイズのフレーム数を置き換える
