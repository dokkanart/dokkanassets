...
setMoveKey(  260,    0,   -100,  150,   0);
setMoveKey(  261,    0,   -100,  150,   0);
setMoveKey(  308,    0,   -1100,  130,   0);
setScaleKey( 261,    0,  1, 1);
setScaleKey( 290,    0,  0.1, 0.1);

kamehame_beam = entryEffectLife( 257, SP_02, 109, 0x40,  0,  300,  0,  300);   -- 伸びるかめはめ波
setEffAlphaKey( 257, kamehame_beam, 0);
setEffAlphaKey( 259, kamehame_beam, 0);
setEffAlphaKey( 260, kamehame_beam, 255);

setEffMoveKey( 370,kamehame_beam,0,  -250,  0);
...
entryFadeBg( 260, 0, 20, 10, 230, 230, 230, 255);    -- 最初だけ明るい　背景
...
entryFade( 350, 15, 10, 10, 255, 255, 255);          -- ベース暗め　背景
...
setMoveKey(  460, 1,  120,    -100,  0);
setMoveKey(  469, 1,  400,    -150,  0);
...
SP_dodge = 441; --エンドフェイズのフレーム数を置き換える
...
-- ** エフェクト等 ** --
aura = entryEffectLife(5,   311, 39, 0x40+0x100,  0,  1,  0,  0); -- オーラ
setEffScaleKey(5, aura, 2.0, 2.0);
...
-- カメハメハ発射(tyo2+110F)
------------------------------------------------------
removeAllEffect( tyo2+257);

changeAnime( tyo2+260, 0, 31);                                    -- かめはめ発射ポーズ

setMoveKey( tyo2+259,    0,      0,  0,   0);
setMoveKey( tyo2+260,    0,   -100,  150,   0);
setMoveKey( tyo2+261,    0,   -100,  150,   0);
setMoveKey( tyo2+308,    0,   -1100,  130,   0);
--setScaleKey( tyo2+261,    0,  1, 1);
--setScaleKey( tyo2+290,    0,  0.1, 0.1);

ryuusen = entryEffectLife( tyo2+260, 920, 109, 0x80,  -1,  0,  0,  0); -- 流線
setEffAlphaKey( tyo2+260, ryuusen, 255);

kamehame_beam = entryEffectLife( tyo2+260, SP_02, 111, 0x40+0x80,  0,  300,  0,  300);   -- 伸びるかめはめ波
setEffMoveKey( tyo2+370,kamehame_beam,0,  -250,  0);
...
setEffAlphaKey( tyo2+260, kamehame_beam, 255);
...
shuchusen = entryEffectLife( tyo2+260, 906, 49, 0x00,  0, 0,  0,  0);   -- 集中線
...
entryFade( tyo2+350, 15, 10, 10, 255, 255, 255);          -- ベース暗め　背景
...
changeAnime( tyo2+370, 1, 102);                        -- ガード
setMoveKey( tyo2+369,    1,  235,  -100,   0);
setMoveKey( tyo2+370,    1,  235,  -100,   0);
setMoveKey( tyo2+371,    1,  235,  -100,   0);
setScaleKey( tyo2+369,    1,  0.7, 0.7);
setScaleKey( tyo2+370,    1,  0.2, 0.2);

setMoveKey( tyo2+400,    1,   120,  0,   0);
setScaleKey( tyo2+400,   1,   0.4,  0.4);

playSe( tyo2+390, SE_06);

kamehame_beam2 = entryEffect( tyo2+370, SP_03,   0x80, -1,  0,  160,  0);   -- 迫るかめはめ波
setEffScaleKey( tyo2+370, kamehame_beam2, 0.3, 0.3);
...
--setEffReplaceTexture( speff, 6, 3);                       -- カットイン差し替え
seId = playSe( tyo2+370, 1044);

-- ** エフェクト等 ** --
setShakeChara( tyo2+370, 1, 99, 20);

entryFadeBg( tyo2+370, 0, 99, 0, 10, 10, 10, 250);          -- ベース暗め　背景

sen1 = entryEffectLife( tyo2+369, 921, 99, 0x80,  -1,  0,  0,  0); -- 流線斜め
setEffRotateKey( tyo2+369,sen1,180);
...
SP_dodge = 185+226; --エンドフェイズのフレーム数を置き換える
...
stopSe( tyo2+470, seId,   4);
entryFade( tyo2+462, 6,  7, 4, fcolor_r, fcolor_g, fcolor_b, 255);     -- white fade

playSe( tyo2+455, SE_09);
-- 敵吹っ飛ぶモーション
changeAnime( tyo2+455, 1, 107);
setMoveKey( tyo2+460, 1,  120,    -100,  0);
setMoveKey( tyo2+469, 1,  400,    -150,  0);
setScaleKey( tyo2+455, 1,  0.4, 0.4);
--setScaleKey( tyo2+458, 1,  0.2, 0.2);
setScaleKey( tyo2+468, 1,  0.5, 0.5);


-- 書き文字エントリー
ct = entryEffectLife( tyo2+370, 10014, 99, 0, -1, 0, -100, 255); -- ズドドッ
setEffShake( tyo2+370, ct, 99, 20);
setEffScaleKey( tyo2+370, ct, 3.4, 3.4);
setEffRotateKey( tyo2+370, ct, -70);
setEffAlphaKey( tyo2+370, ct, 255);
setEffAlphaKey( tyo2+430, ct, 255);
setEffAlphaKey( tyo2+450, ct, 0);

entryFade( tyo2+450, 10, 8, 15, 255, 255, 255);          -- ベース暗め　背景
...
setScaleKey( tyo2+469,    1,  0.5, 0.5);
